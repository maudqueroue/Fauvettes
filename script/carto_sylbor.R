...
# Les differents points avec leur longitude et latitude
...
geo_STOC <- dplyr::left_join(sylbor_STOC, geo_STOC, by = c("ID_PROG")) %>%
...
geo_EPS <- dplyr::left_join(sylbor_EPS, geo_EPS, by = c("point")) %>%
  dplyr::filter(!is.na(long)) %>%
  dplyr::filter(!is.na(lat))
...
model <- gamm4(round(n)~s(lat),family=poisson,data=geo_EPS)
